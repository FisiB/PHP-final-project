<?php 
   ...
   if ($_SESSION['isadmin'] == 'true') {
     ...
     $sql = "SELECT iteams.product_name, users.email,buy.id, buy.nr_items, buy.size FROM iteams
     INNER JOIN buy ON iteams.id = buy.product_id
     INNER JOIN users ON users.id = buy.user_id";
    ...
    $bookings_data = $selectBookings->fetchAll();
      ...
      $sql = "SELECT iteams.product_name, users.email, buy.nr_items, buy.size
            FROM iteams INNER JOIN buy ON iteams.id = buy.product_id 
            INNER JOIN users ON users.id = buy.user_id WHERE buy.user_id = :user_id";
  ...
  <div class="row">
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      ...
      <ul class="nav flex-column">
          ...
          <li class="nav-item">
            <a class="nav-link" href="buying.php">
              <span ></span>
              Bookings
            </a>
          </li>
        ...
        <table class="table table-striped table-sm">
          ...
          <?php if ($_SESSION['isadmin'] == 'true') { ?>
            <?php foreach ($bookings_data as $booking_data) { ?>
               ...
               <tr>
                <td><?php echo $booking_data['product_name']; ?></td>
                <td><?php echo $booking_data['email']; ?></td>
                <td><?php echo $booking_data['nr_items']; ?></td>
                <td><?php echo $booking_data['size']; ?></td>
           <?php }}else{ ?>
            <?php foreach ($bookings_data as $booking_data) { ?>
            <tr>
            <td><?php echo $booking_data['product_name']; ?></td>
